...
# c:/Python/doit/sub_dir_search.py
...
### search 함수를 만들고 시작 데렉터리를 입력받도록 코드를 작성했다.
...
### 여기에서 구하는 파일 리스트는 파일 이름만 포함되어 있으므로 
...
### 위 코드는 확장자 이름이 .py인 경우만을 출력하도록 작성했다. C:/디렉터리에 파이썬 파일이 없다면 아무것도 출력되지 않을 것이다.




### 4.  하지만 우리가 원하는 것은 C:/디렉터리 바로 밑에 있는 파일뿐만 아니라 
### 그 하위 디렉터리(sub directory)를 포함한 모든 파이썬 파일을 검색하는 것이다.
### 하위 디렉터리도 검색이 가능하도록 다음과 같이 코드를 변경해야 한다.

# c:/Pyhton/doit/sub_dir_search.py
...
### try ... except PermissionError로 함수 전체를 감싼 이유는
### os.listdir를 수행할 때 권한이 없는 디렉터리에 접근하더라도 프로그램이 오류로 종료되지 않고 그냥 수행되도록 하기 위해서이다.
### full_filename이 디렉터리인지 파일인지 구별하기 위하여 os.path.isdir 함수를 사용하였고 
### 디렉터리일 경우 해당 경로를 입력받아 다시 search 함수를 호출하였다. 
### 이렇게 해당 디렉터리의 파일이 디렉터리일 경우 다시 search 함수를 호출해 나가면 (재귀 호출) 해당 디렉터리의 하위 파일을 다시 검색하기 시작하므로
### 결국 모든 파일들을 검색할 수 있게 된다.


# ※ 재귀 호출이란 자기 자신을 다시 호출하는 프로그래밍 기법이다. 
# 이 코드에서 보면 search 함수에서 다시 자기 자신인 search 함수를 호출하는 것이 바로 재귀 호출이다.

### 위 코드를 수행하면 C:/ 디렉터리에 있는 모든 파이썬 파일이 출력돨 것이다.
